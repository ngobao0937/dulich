...
<?php
    ...
    $product_fk = $room->product_fk ?? null;
    $active = $room->active ?? '';
    $meta_keywords = $room->meta_keywords ?? null;
    $meta_description = $room->meta_description ?? null;
    $image = isset($room->image) ? $room->image->ten : null;
    $vr360 = isset($room->vr360) ? $room->vr360->ten : null;
    $images = $room->images ?? null;
?>
<section class="content-header">
    <div class="container-fluid">
        <form action="{{ route('backend.room.store', request()->query()) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-2">
                            <div class="form-group">
                                ...
                                <input type="number" min="0" name="price" id="price" class="form-control" value="{{ $price }}" maxlength="250" required>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="select2-primary">
                            <select name="extensions_fk[]" id="extensions_fk" class="select2-multiple" multiple="multiple" data-placeholder="-- Chọn các tiện ích --">
                                @php
                                    $selectedExtensions = (isset($room) && $room != null) ? $room->extensions->pluck('id')->toArray() : [];
                                @endphp
                                @foreach (\App\Models\Extension::all() as $extension)
                                    <option value="{{ $extension->id }}" 
                                        {{ in_array($extension->id, $selectedExtensions) ? 'selected' : '' }}>
                                        {{ $extension->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    @if(isset($room) && $images && count($images) > 0)
                    ...
                    <div class="form-group">
                        <label>Meta Description</label><i class="fas fa-question-circle ml-1" data-toggle="tooltip" title="Hiển thị nội dung mô tả khi chia sẻ liên kết phòng qua Messenger, Zalo, ..."></i>
...
<script>
    ...
    $('#myModal button.delete').on('click', function(e) {
       ...
       $.ajax({
            url: '{{ route('backend.room.delete.img') }}',
            method: 'delete',
            data: {
                id: imageId,
            },
